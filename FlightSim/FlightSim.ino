...
uint8_t Platform::GetInput()
{
  ...
  if(arduboy.pressed(UP_BUTTON)||Y>10)
  {
    result |= INPUT_UP;  
  }
  if(arduboy.pressed(DOWN_BUTTON)||Y<-10)
  {
    result |= INPUT_DOWN;  
  }
  if(arduboy.pressed(LEFT_BUTTON)||X<-10)
  {
    result |= INPUT_LEFT;  
  }
  if(arduboy.pressed(RIGHT_BUTTON)||X>10)
  {
    result |= INPUT_RIGHT;  
  }
